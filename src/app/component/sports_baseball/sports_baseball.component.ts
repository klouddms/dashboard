...
@Component({
  selector: 'sports_baseball',
  templateUrl: './sports_baseball.template.html',
})

export class SportsBaseballC implements OnInit,AfterViewInit {
  options;
  data;
  @ViewChild(nvD3)
  nvD3: nvD3;
  ...
  constructor(private router: Router, private userServiceIns: UserService, private gameServiceIns: GameService, private evaluationServiceIns: EvaluationService, private refereeServiceIns: RefereeService, private leagueServiceIns: LeagueService, private locationServiceIns: LocationService, private sportsServiceIns: SportsBaseMethod , private tableMethodIns:TableBaseMethod, private globalIns: GlobalVariableService) {
    this.tabNameAndIdVal = this.sportsServiceIns.getTabDataBySubMenuMed(this.leagueServiceIns.getLeagueJsonDataMed(), 'Baseball', null);
  ...
  ngAfterViewInit() {
    _.forEach(this.tabNameAndIdVal, (n) => {
      this.tableMethodIns.drawDataTableMed('#baseball-sports-upcoming-table'+n.id);
      this.tableMethodIns.drawDataTableMed('#baseball-sports-recent-table'+n.id);
    })
  }
  ...
  getTableIdMed(uORParam:boolean, tabContentParam: any){
    if(tabContentParam != null || tabContentParam != undefined){
      if(uORParam){
        return `baseball-sports-recent-table${tabContentParam.id}`;
      }else{
        return `baseball-sports-upcoming-table${tabContentParam.id}`;
      }
    }
  }
   ...
   gotoRowDetailMed(row){
     ...
     this.globalIns.setPreviousRouteMed('app/sports_baseball');
